...
| **FUARAN069** | Warning | An interactive control (form field / Select / Tabs / dismissable Modal / Disclosure) has **no event handler and no writable value binding** – the Phase 426 write-back default has nothing to write to, so the control is inert. Bind the control's value slot directly to `{"$type":"State","key":…}` (or `{"$type":"Filter","name":…}`), or supply the handler. **`Rating` is exempt** (Phase 1648): a handler-less rating with a static value is a *read-only rating*, which both renderers implement as a designed rendition – `fuaran-rating-static` with `role="img"` and the score as its `aria-label`, rather than a slider nobody can move – so "4 stars, as scored" is a legitimate thing for a form to show. `Color` is not exempt: `<input type="color">` is the control on every host and has no static rendition. |
...
| **FUARAN086** | Error | A `ChartSpec` names a field its own source cannot **produce** — `xField`, a `yFields` entry, or an annotation's field, absent from the schema the `Transform` the chart reads produces (Phase 640, widened to the whole pipeline by Phase 1486). Nothing throws: the lowering reads the missing key as absent and draws a **silently flat or empty series**, which looks like data. Fix the name, or change the pipeline so it produces the column — the message lists the columns it does produce. Same produced-schema derivation, and the same stand-downs, as **FUARAN114** below, which is this rule on the read side of a grid: `derive` **adds** a column, `project` / `groupBy` / `unpivot` **close** the set to what they name, and a source whose column set cannot be closed (`$ref`, `pivot`) or is not the tree's to know (`$state`, `$query`, host `$static`) is not reported at all. A field that IS produced but is the wrong TYPE is `FUARAN087`, and a temporal x-axis over a non-date column is `FUARAN097`. |
| **FUARAN114** | Error | A `DataGrid` names a column its own source cannot **produce** — a column `field`, or the grid's `rowKeyField`, absent from the schema the `Transform` the grid reads produces (Phase 1149, widened to the whole pipeline by Phase 1486). The row projection finds nothing, so the cell renders blank (or, for `rowKeyField`, every row shares one empty key and row identity collapses). Fix the name, or change the pipeline so it produces the column. The produced schema is derived by folding the pipeline over the source's own: `derive` **adds** a column (so a grid may bind it), `project` / `groupBy` / `unpivot` **close** the set to exactly what they name (so a grid still binding a renamed-away column is refused, and the message lists the produced columns). The rule stands down wherever the column set cannot be closed — a `$ref` source the host resolves, a `pivot` whose value columns are named by the data — and a `$state`, `$query` or host `$static` source is never reported at all. |
...
> **One row here is a deliberate near-copy of the string a rule emits, and it is coupled by hand.**
> `FUARAN064`'s row restates the message `src/Fuaran.UI.Validator/ButtonDisabledCheck.fs` prints —
> same diagnosis, same remedy, same `Binding.Static true` carve-out — because this table is what an
> author reads *before* writing the button and that string is what they read *after*. Nothing
> enforces the agreement: the cell is prose with code spans in it and the emitted string is a single
> log line, so a byte-parity gate would have to make one of them worse. Declared instead, at both
> ends (Phase 1646): a change to that rule's diagnosis, remedy or carve-out edits both in the same
> change-set. Wording alone need not — the two were never byte-identical and are not claimed to be.
...
The renderer projects each `field` off the row (a `Transform` produces rows keyed by column name). A column with **neither** `value` nor `field` renders blank – always give a decoded column a `field`. **Every `field` and the `rowKeyField` must name a column the source can actually produce: one that does not is `FUARAN114` (error) wherever the produced column set can be derived** – a `Transform` over an inline `$data` table, its whole pipeline folded through, so a `derive`d column counts and a `project`ed-away one does not – because the projection would otherwise find nothing and the cell would render blank, indistinguishable from an empty value. The rule stands down where the set cannot be closed (a `$ref` source, a `pivot`), and never fires over a `$state`, `$query` or `$static` source.
