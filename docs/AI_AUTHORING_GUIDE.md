...
| **FUARAN114** | Error | A `DataGrid` names a column its own source cannot produce — a column `field`, or the grid's `rowKeyField`, absent from the statically-known schema of the `Transform` the grid reads (Phase 1149). The row projection finds nothing, so the cell renders blank (or, for `rowKeyField`, every row shares one empty key and row identity collapses). Fix the name, or source the grid from data that carries the column. The schema is statically known for a `Transform` over an inline `$data` table with an **empty** pipeline; a non-empty pipeline (`derive` adds columns, `project`/`groupBy` remove them), a `$state`, `$query` or host `$static` source is unknowable before the tree runs and is deliberately never reported. |
...
The renderer projects each `field` off the row (a `Transform` produces rows keyed by column name). A column with **neither** `value` nor `field` renders blank – always give a decoded column a `field`. **Every `field` and the `rowKeyField` must name a column the source actually carries: one that does not is `FUARAN114` (error) wherever the source's schema is statically known** – a `Transform` over an inline `$data` table with an empty pipeline – because the projection would otherwise find nothing and the cell would render blank, indistinguishable from an empty value.
