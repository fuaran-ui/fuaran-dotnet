...
|---|---|---|
| `NavBar` / `Menu` | **Kind** (reserved) | Distinct `<nav>` landmark + menu ARIA semantics; no existing kind carries navigation-landmark semantics. Strongest genuine-kind candidate in this cluster. |
...
| `Pagination` | **Composition** | `Button` + `SetState` over a page-index state key; no new vocabulary. |
