...
- **`FormFieldKind`** absorbed date/time (`Date` + `DateVariant`), ranged number (`RangedNumber`),
  ...
  (`Combobox` is no longer future — it shipped at 0.51.0 under exactly this ruling; see its
  Appendix A row.)
...
| `Rating` / `ColorPicker` | **Variant** | Both `FormFieldKind` variants – data entry is the field DU's territory. |
