...
Every public law family the pinned `Fuaran.Core.Conformance` and `Fuaran.Core.DataFrame.Conformance` **0.32.0** ship, and how this repo answers for it.
...
|---|---|---|
| `Conformance.aggregateNullSkipLaws` | Adopted | `aggregate null-skipping certifies under Core's aggregateNullSkipLaws` — via `Conformance.aggregateNullSkipLaws` |
| `Conformance.aggregateParityLaws` | Adopted | `aggregate parity certifies under Core's aggregateParityLaws` — via `Conformance.aggregateParityLaws` |
...
| `Conformance.aiSurfaceLawsUnderKitPolicy` | Not used | Fuaran.Core.AiSurface's proposal plumbing — no project in this tier references Fuaran.Core.AiSurface or submits an op through its Decide (see aiSurfaceLaws) |
...
| `Conformance.capabilityLawsWith` | Carried by phase | unfiled — Fuaran.UI.AiTools.Capabilities (validate, then makeInvoker's host body) is a host dispatch path over a Core CapabilityRegistry, the subject this family certifies; adoption is one CapabilitySeamWitness whose Dispatch is that path, in Fuaran.UI.AiTools.Tests |
...
| `Conformance.capabilityPipelineLawsWith` | Not used | Fuaran.Core.Function's CapabilityPipeline at a domain's CapabilityPipelineWitness — the tier composes no capability pipeline |
...
| `Conformance.chainBreakReasonLaws` | Adopted | `the kit's classified chain walkers never mint Unrecognised (chainBreakReasonLaws)` — via `Conformance.chainBreakReasonLaws` |
| `Conformance.codecInjectivityLaws` | Not used | op-codec injectivity over an `'Op : equality` witness — `TreeOp<obj>` is obj-erased and has no equality to state distinctness with; the codec round-trips byte-stably against the wire corpus (`JsonDecode.Tests`, fuaran-ts parity), which is the same claim in the form this op type admits |
...
| `Conformance.columnarOpLawsWith` | Adopted | `the columnar op algebra certifies under Core's columnarOpLawsWith at the kit's reference generator` — via `Conformance.columnarOpLawsWith — at ColumnOps.invert and Conformance.columnarOpStreamGen, the kit's reference generator; the tier has no table-edit op to draw its own` |
...
| `Conformance.compositionPilot` | Not used | Fuaran.Core.Function's applyMemo over a cross-witness composeAcross — the tier supplies one ArtifactWitness and no memoised cross-witness composition, so the pilot has no second witness or memo key to run against |
...
| `Conformance.constructThenEncodeLaws` | Carried by phase | unfiled — fuaran-core#126 handed the first adoption to this tier's F# smart constructors (the codec and the Corpus.Case list already exist, so the adoption is one ConstructWitness); the row flips to Adopted when that witness is supplied |
| `Conformance.containerLaws` | Adopted | `the Fuaran.UI container predicate certifies under Core's containerLaws` — via `Conformance.containerLaws` |
| `Conformance.dagBreakReasonLaws` | Adopted | `the kit's classified DAG walker never mints Unrecognised (dagBreakReasonLaws)` — via `Conformance.dagBreakReasonLaws` |
...
| `Conformance.diffContainedLaws` | Not used | `Diff.toOpsContained` under a `CanHold` — this tier applies structural ops through `applyContained` but derives no contained diff script; `Diff.toOps` is certified through `certify` |
...
| `Conformance.incrementalLawsWith` | Not used | DataFrame.evalFrom over ColumnOps.changeOf at a domain's StreamGen<ColumnOp, Table> — the tier's incremental path is Incremental.primeOn / refreshOn over a row TableDelta (ServerDriven/LiveTransform.fs, see IncrementalDelta.laws / lawsWith), and no project here edits a table through a ColumnOp |
...
| `Conformance.nowLaws` | Adopted | `nowLaws certifies the pinned evaluation clock at this pin` — via `Conformance.nowLaws` |
| `Conformance.opAlgebra` | Adopted | `the Fuaran.UI witness certifies end-to-end via the unified Conformance.certify` — via `Conformance.certify` |
...
| `Conformance.propagationEvaluatorLaws` | Not used | Fuaran.Core.Propagation's incremental evaluator seam (Propagation.eval / evalFrom) — no project in this tier references Fuaran.Core.Propagation or hands it an evaluator; the tier's reactivity runs on its own stores (see propagationEvalLaws) |
| `Conformance.propagationEvaluatorLawsWith` | Not used | Fuaran.Core.Propagation's prior-aware evaluator seam (Propagation.evalWith / evalFromWith) — no project in this tier references Fuaran.Core.Propagation or hands it an evaluator (see propagationEvaluatorLaws) |
...
| `Conformance.queryLawsWith` | Not used | Fuaran.Core.Query's registry seam at a domain's QuerySeamWitness — no project here references the Core query registry or dispatches through QueryRegistry.dispatch (see queryLaws) |
...
| `Conformance.reducer` | Adopted | `the Phase-406 StreamEntry witness certifies via Conformance.certifyStream (consumption)` — via `Conformance.certifyStream` |
...
| `Conformance.schemaWalkLaws` | Adopted | `static output-schema derivation certifies under Core's schemaWalkLaws` — via `Conformance.schemaWalkLaws` |
| `Conformance.slotParamLaws` | Adopted | `slotParamLaws certifies a param in a Sort / Limit slot at this pin` — via `Conformance.slotParamLaws` |
...
| `FoldConfluence.laneFoldLaws` | Adopted | `N-lane folding is arrival-order-invariant under Core's laneFoldLaws` — via `FoldConfluence.laneFoldLaws (.NET)`<br>`laneFoldLaws certifies through the Fable law harness over the tier's reducer and codec` — via `FoldConfluence.laneFoldLaws (Fable and Node)` |
