...
### Palette attribution order — the manifest tier's one ordering contract (Phase 1727)

A manifest routinely carries **two colour tokens with the same value**. That is not a defect: it is the ordinary shape of a projected surface (shadcn's `--primary` and `--ring` are one value under two names; a DTCG alias resolves to the value it aliases; a brand palette keeps `color.link` equal to `color.brand` on purpose). It does create one question the manifest tier has to answer deterministically. The **usage-budget** check (`ManifestFlags.verifyUsageBudgets`, and its Python / Go / Rust ports) attributes each rendered fill to ONE palette token — the token whose area share a `UsageBudget` invariant measures — and when two tokens carry the fill's value, *which* token takes the area decides which budget breaches. (The per-node `OffPaletteColour` check is a membership test and is unaffected; only attribution is order-sensitive.)

**The rule.** Palette attribution iterates the manifest's colour tokens in **canonical token-path order**, and the **first token whose value matches wins**. Canonical token-path order compares dotted paths **segment by segment** — a shorter prefix first, each segment by Unicode code point (UTF-8 byte order) — which is exactly the order a depth-first walk of the DTCG token tree yields when it visits every group's members in ascending key order. Two consequences are worth stating because both have been got wrong:

- **Document order plays no part.** A manifest declaring `color.secondary` before `color.brand`, both `#3b5bdb`, attributes a `#3b5bdb` fill to `color.brand`.
- **It is not a sort of the dotted string.** `color.brand.base` precedes `color.brand-alt`, because the key `brand` precedes `brand-alt`, even though `-` sorts before `.` as a character. A host that sorted the joined path would put `brand-alt` first and diverge from every host that sorts per group.

**Who has to do what.** The attribution is the contract; the mechanism is not. A host satisfies the rule either by yielding tokens from its DTCG walk in canonical order (the Go and Rust decoders sort each group's keys) or by ordering the palette at the attribution site (this host's `ManifestFlags` and the Python `manifest_flags` do, because both decoders preserve document order and a projection consumer may rely on that). A decoder that yields tokens in another order is **not** thereby non-conformant — lookup by name makes order irrelevant everywhere else — but a host whose *attribution* follows that order is non-conformant, and the corpus says so: `style-observer/budget-same-valued-tokens-attribute-path-first` and `…-order-is-segment-wise` (both `usage-budget` tier) carry a deliberate tie declared in the reverse of canonical order, and every host with an observer tier certifies against them.

**The alternative that lost.** Refusing a manifest carrying two same-valued colour tokens — treating the tie as a manifest defect at decode — was considered and rejected. It would make every projected surface in Example B below a defect on arrival; it would have to be enforced in four decoders and four projectors, none of which can avoid emitting one value under two names; and it answers the wrong question, because a budget on an alias token is a legitimate thing to declare. A rule costs one sentence and is checkable from the token names alone; a prohibition costs every host a refusal path and every author a workaround. Document order was the other candidate, and it lost on reachability: the Go decoder reads the token tree through a map that has no document order to preserve, so a document-order rule could not be implemented on every host, while path order is computable everywhere from the names alone.
