...
```json
{"id":"metric-1","kind":{"$type":"Metric","format":{"$type":"Currency","code":"GBP"},"icon":"trending-up","label":"Revenue","subtext":"vs last month","tone":"Brand","trend":0.07,"trendFormat":{"$type":"Percent","decimals":1},"value":1234.5}}
```
<!-- /fuaran:example -->
...
```json
{"id":"composite-root","kind":{"$type":"Box","children":[{"id":"composite-card","kind":{"$type":"Box","children":[{"id":"metric-2","kind":{"$type":"Metric","format":{"$type":"Currency","code":"GBP"},"icon":"trending-up","label":"Revenue","subtext":"vs last month","tone":"Brand","trend":0.07,"trendFormat":{"$type":"Percent","decimals":1},"value":1234.5}},{"id":"lvr-1","kind":{"$type":"LabelValueRow","emphasis":true,"format":{"$type":"Number","decimals":2},"help":"Last 30 days","label":"Total","value":42}}],"heading":"Composite","layout":{"$type":"Flex","direction":"Vertical","wrap":false},"role":"Card"}},{"id":"stack-1","kind":{"$type":"Box","children":[{"id":"metric-1","kind":{"$type":"Metric","format":{"$type":"Currency","code":"GBP"},"icon":"trending-up","label":"Revenue","subtext":"vs last month","tone":"Brand","trend":0.07,"trendFormat":{"$type":"Percent","decimals":1},"value":1234.5}},{"id":"markdown-1","kind":{"$type":"Markdown","text":"Updated hourly."}}],"layout":{"$type":"Flex","direction":"Vertical","wrap":false},"role":"Group"}}],"layout":{"$type":"Auto"},"role":"Dashboard"}}
```
<!-- /fuaran:example -->

**A wrapper's `children` are usually containers, not leaves.** `Tabs`, `Disclosure`,
`SplitPanel`, `Stepper`, `Modal` and `ScrollArea` take the same `children` array as `Box`,
so a tabbed screen is a `Tabs` whose children are `Box` cards — one panel per tab, each
laying out its own contents. Dropping bare leaves straight into a wrapper is the commonest
structural miss. The example below is also the pattern for a second thing: **when the
request names a control's starting value, that control arrives PRE-FILLED** — give its
`value` a `Static` binding rather than omitting the slot.

<!-- fuaran:example fixture=composite-tabs-panels -->
```json
{"id":"composite-tabs-panels","kind":{"$type":"Tabs","activeIndex":0,"activeTag":"overview","children":[{"id":"overview-panel","kind":{"$type":"Box","children":[{"id":"spark-1","kind":{"$type":"Sparkline","source":[1,2,3,2,4]}},{"id":"badge-1","kind":{"$type":"Badge","label":"Beta","variant":"Info"}}],"heading":"This month","layout":{"$type":"Grid","cols":2,"gap":16},"role":"Card"}},{"id":"settings-panel","kind":{"$type":"Box","children":[{"id":"preferences-form","kind":{"$type":"Form","fields":[{"id":"displayName","kind":{"$type":"Text","value":"Ada Lovelace"},"label":"Display name","required":true},{"id":"theme","kind":{"$type":"Choice","options":[{"label":"Light","value":"light"},{"label":"Dark","value":"dark"}],"value":"dark"},"label":"Theme","required":true}],"onSubmit":{"$type":"Call","endpoint":"/api/preferences"},"submitLabel":"Save preferences"}}],"heading":"Preferences","layout":{"$type":"Flex","direction":"Vertical","gap":12,"wrap":false},"role":"Card"}}],"tabHeaders":[{"icon":"chart-glyph","label":"Overview"},{"disabled":false,"label":"Settings"}],"tabTags":["overview","settings"]}}
```
<!-- /fuaran:example -->
...
| Chart { kind:"Line"|"Bar"|"Area"|"Pie"|"Scatter"|"Heatmap"; source:Binding; xField:str; yFields:str[]; dataLabels?:"Off"|"Ends"; legendPosition?:"Top"|"Right"|"Bottom"|"None"; stacked?:bool; subtitle?:TextSource; title?:TextSource; valueFormat?:Format; xTitle?:TextSource; yTitle?:TextSource }
...
```json
{"id":"filterable-static-dashboard","kind":{"$type":"Box","children":[{"id":"content-filters","kind":{"$type":"Filters","items":[{"kind":{"$type":"Choice","options":[{"label":"EMEA","value":"emea"},{"label":"Americas","value":"amer"}]},"label":"Region","name":"region"},{"kind":{"$type":"Choice","options":[{"label":"Drama","value":"drama"},{"label":"Documentary","value":"docs"}]},"label":"Genre","name":"genre"}]}},{"id":"retention-chart","kind":{"$type":"Chart","kind":"Line","source":{"$type":"Transform","params":[{"from":{"$type":"Filter","name":"region"},"name":"region"},{"from":{"$type":"Filter","name":"genre"},"name":"genre"}],"pipeline":[{"$type":"filter","column":"region","op":"eq","param":"region"},{"$type":"filter","column":"genre","op":"eq","param":"genre"}],"source":{"columns":{"genre":["drama","docs"],"month":["jan","jan"],"region":["emea","amer"],"retention":[0.62,0.55]}}},"stacked":false,"title":"Retention","xField":"month","yFields":["retention"]}},{"id":"episode-grid","kind":{"$type":"DataGrid","columns":[{"field":"month","kind":{"$type":"Text"},"label":"Month"},{"field":"retention","kind":{"$type":"Text"},"label":"Retention"}],"rowKeyField":"month","source":{"$type":"Transform","params":[{"from":{"$type":"Filter","name":"region"},"name":"region"},{"from":{"$type":"Filter","name":"genre"},"name":"genre"}],"pipeline":[{"$type":"filter","column":"region","op":"eq","param":"region"},{"$type":"filter","column":"genre","op":"eq","param":"genre"}],"source":{"columns":{"genre":["drama","docs"],"month":["jan","jan"],"region":["emea","amer"],"retention":[0.62,0.55]}}}}}],"heading":"Content performance","layout":{"$type":"Auto"},"role":"Dashboard"}}
```
<!-- /fuaran:example -->
...
```json
{"id":"now-environment-binding","kind":{"$type":"Box","children":[{"id":"today-fact","kind":{"$type":"Fact","label":"Today","value":{"$type":"Bound","binding":{"$type":"Now"}}}},{"id":"overdue-grid","kind":{"$type":"DataGrid","columns":[{"field":"id","kind":{"$type":"Text"},"label":"Invoice"},{"field":"due","kind":{"$type":"Text"},"label":"Due"},{"field":"daysOverdue","kind":{"$type":"Text"},"label":"Days overdue"}],"rowKeyField":"id","source":{"$type":"Transform","params":{"today":{"$type":"Now"}},"pipeline":[{"$type":"derive","expr":{"$type":"apply","args":[{"$type":"param","name":"today"},{"$type":"col","name":"due"}],"fn":"dateDiffDays"},"name":"daysOverdue"}],"source":{"columns":{"due":["2026-07-01","2026-07-28"],"id":["INV-1001","INV-1002"]},"schema":[{"name":"id","type":"string"},{"name":"due","type":"string"}]}}}}],"layout":{"$type":"Auto"},"role":"Dashboard"}}
```
<!-- /fuaran:example -->
...
```json
{"id":"switch-on-selection","kind":{"$type":"Box","children":[{"id":"ward-grid","kind":{"$type":"DataGrid","columns":[{"field":"id","kind":{"$type":"Text"},"label":"Ward"},{"field":"status","kind":{"$type":"Text"},"label":"Status"}],"rowKeyField":"id","source":{"$type":"Transform","pipeline":[],"source":{"columns":{"id":["WARD-A","WARD-B"],"status":["steady","critical"]}}}}},{"id":"ward-status-panel","kind":{"$type":"Switch","cases":[{"child":{"id":"ward-critical","kind":{"$type":"Callout","body":"Escalate admissions to the on-call manager.","heading":"Ward at capacity","tone":"Critical"}},"match":"critical"}],"default":{"id":"ward-steady","kind":{"$type":"Markdown","text":"Occupancy within normal range."}},"on":{"$type":"Selection","defaultValue":"steady","field":"status","nodeId":"ward-grid"}}}],"layout":{"$type":"Auto"},"role":"Dashboard"}}
```
<!-- /fuaran:example -->
