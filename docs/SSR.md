...
| **Meta.Switch — `autoAdvanceMs`** (Phase 1122) | **The floor is the bound case's child, rendered once, with NO TIMER — and that is the conforming answer rather than a gap.** A no-script host resolves the switch's selector from the state the host seeded, renders the matching case (else the `Default`), and stops. Nothing advances, because advancing means writing a state key on an interval and a static document has neither an interval nor a writer; emitting a stage that looked as though it would move would be an invitation the document cannot honour. The declaration is **not dropped silently**: nothing about the emitted markup differs from a switch with no interval, which is the honest rendering, and the interval reaches the client tier on the wire where it is acted on. **The stage wrapper is client-only.** The pause, stop, swipe and arrow-key affordances all need listeners and state, so there is no inert markup that could carry them, and the server emits the selected child exactly as it did before this phase — which is also what makes hydration mismatch-free: the client's first render, before its own reduced-motion read, produces the same tree. **The two transition tokens are unaffected** — `motion` is host-only (`WIRE_FORMAT.md` §9) and never reaches a server-rendered tree at all. |
| **Visualisation.DataGrid — `transferOutKey` / `transferInKey`** (Phase 1123) | **The floor is the grid rendered exactly as it is rendered without the pair, and that is the conforming answer rather than a gap.** A no-script host emits the rows it resolved and nothing else: no drag handle it could not begin a drag from, no place control it could not activate, and no live region it could never write into. A transfer is a gesture plus two state writes, and a static document has neither a gesture nor a writer — emitting the chrome inert would advertise a move the page cannot perform, which is the fake-affordance failure the admitting charter row exists to foreclose. **The declaration is not dropped silently:** both keys ride the wire to the client tier, where they are acted on, and the SSR markup of a transfer-declaring grid is byte-identical to the same grid without the pair. **Consequently there is no hydration question here at all** — the client's first render adds the chrome, and it is adding markup the server never claimed rather than reconciling markup the two pipelines describe differently. |
| **Input.Button — `Action.Print`** (Phase 1124) | **The floor is the control rendered exactly as any other inert button, and that is the conforming answer rather than a gap.** A no-script host emits the button with its label, its classes and its `disabled` state and attaches nothing — the general Input rule three rows up, reached for one more action case. It is worth stating separately only because the obvious alternative is wrong: **a server-rendered page has no printer, so `Action.Print` is not an action a server can perform on the reader's behalf, and it is not one it may refuse either.** Printing is an act of the machine the page is being READ on. A server that treated the declaration as unserviceable would be answering a question about its own process when the question is about the reader's. **On the server-DRIVEN path the effect is therefore LOWERED, not executed and not refused** — `Driver.interpret` maps it to `ClientEffect.Print` and the shim calls `window.print()`, exactly as it does for `Action.WriteToClipboard`, which is the same shape of fact (a browser capability the server names and the client performs). **On the RESUMABILITY path it is an `interpret` disposition** (`Resume.disposition`): it carries no closure, needs no module chunk and consults no host, so a resumed page performs it directly with no framework JavaScript executed. **Nothing round-trips back.** `window.print()` reports neither whether the reader printed nor what they chose, so unlike `ClientEffect.ReadFileBody` there is no result `LiveEvent`, and a server never learns that a page was printed. **The PAGED RENDERING itself is not on this path at all** — it is the reference stylesheet's `@media print` block, which a static no-script page carries and honours in full, so the server-rendered document prints correctly whether or not any action ever fires. |
| **Input.Button — `Action.Confirm` / `Action.Focus`** (Phase 1537) | **The floor is the control rendered exactly as any other inert button, and for both cases that is the conforming answer rather than a gap** — the general Input rule, reached for two more action cases, and the print row above is the shape. It is worth stating separately for the same reason that row is: the obvious alternatives are both wrong. **A static host must not render the question**, as a paragraph, a `<details>` or anything else. A confirmation is a QUESTION WHOSE ANSWER IS DISPATCHED, and a document that cannot dispatch has nowhere to put the answer; showing the prompt would advertise a decision the page cannot act on, which is the fake-affordance failure the admitting charter row exists to foreclose. **And it must not perform the continuation either** — a static host that "assumed yes" would run the destructive act the question exists to guard, which is worse than doing nothing by exactly the margin the case was admitted for. **`Focus` is silently absent for a simpler reason:** there is no gesture to move the caret in response to, and moving it at load time would be a focus steal the reader never asked for. Nothing about the initial focus of a static document is on this path at all. **On the server-DRIVEN path both are LOWERED, not executed and not refused**, exactly as `Print` and `Action.WriteToClipboard` are: `Driver.interpret` maps `Focus` to the `ClientEffect.Focus` this channel has carried since Phase 152, and `Confirm` to `ClientEffect.Confirm(prompt, token)` — the resolved question and an opaque token, and **not** the continuations, which stay on the server so the shim is never told what a yes will do. The answer round-trips as the originating event re-delivered with two extra payload members, is re-validated in full, and the continuation then meets its own dispatch gate. **On the RESUMABILITY path `Focus` is an `interpret` disposition** — no closure, no module chunk, no host — while a `Confirm` interprets only when its prompt is LITERAL and its continuations do; a bound prompt is `fallback`, because the zero-JS interpreter holds no resolver and would put the declaration on screen as the question. **Nothing round-trips back on the static path**, and a server never learns that a reader was asked anything. |
...
| **Visualisation.DataGrid — `exportable`** (Phase 1125) | **The floor is the grid rendered exactly as it is rendered without the declaration, and that is the conforming answer rather than a gap.** A no-script host emits the rows it resolved and no export control: no button it could not serialise from, and no link whose target it could not construct. **An export is a gesture plus a file made on the reader's machine, and a static document has neither a gesture nor a way to make a file** — the CSV is built from the rows the CLIENT holds, in the order the READER has sorted them into, and the server holds neither of those facts at the moment they matter. Emitting the control inert would advertise a download the page cannot perform, which is the fake-affordance failure the admitting charter row exists to foreclose, and it is worse here than elsewhere because a button labelled *Export* that does nothing reads as a broken page rather than as a degraded one. **The declaration is not dropped silently:** the flag rides the wire to the client tier, where it is acted on, and the SSR markup of an exporting grid is byte-identical to the same grid without the flag — so there is no hydration question at all, the client's first render ADDING markup the server never claimed rather than reconciling markup the two pipelines describe differently. **The no-script reader is not left without a route**, and this is why the floor is honest rather than merely defensible: what the export hands over is the grid's own rendered rows, and a static host has already emitted every one of them as a real `<table>` — a reader can select and copy it, and a host that wants a file offers one in its own chrome from the data it served. **On the server-DRIVEN path the control is likewise the client's**, for the reason the print row above records at a neighbouring slot: the rows being serialised are the ones the reader's own browser resolved and re-ordered, so a server answering for its own copy would be answering a question about a different grid. |
...
| **Display.Sparkline** (Phase 1098) | The `fuaran-sparkline` hook wrapping the **real lowered geometry** — a `Sparkline → Drawing` lowering (`Charts.tryLowerSparkline`) emitted through the shared `DrawingSvg` builder, so the server and client bytes are identical by construction. An **unresolved or empty series** keeps the em-dash placeholder. No client-only tier: `render-fidelity.json` records this kind as `"class": "none"`, as it does `Drawing`. |
...
| **Input.Rating** (Phase 1130) | **Two markups, chosen by what the document can honour, and the difference is the statement rather than an inconsistency.** A rating that CANNOT be written — no handler, and a value binding the write-back default cannot reach, which is the bound-average display case — has no interaction to floor, so the SSR tier emits the **identical** `<span role="img">` star row the client renders, from the same `RatingModel.fills`, with the whole reading (`"4.3 out of 5"`) as its `aria-label`. The two tiers agree byte-for-byte because there is nothing for hydration to add. A **writable** rating floors on **native radios** — one `<input type="radio" name="{fieldId}">` per enterable position, inside `<span class="fuaran-rating fuaran-rating-choices">`, beside the same star row. Zero-JS, a `<span role="slider">` can be neither adjusted nor submitted; radios are keyboard-adjustable and submit with the form, and the user agent supplies the group semantics itself. **No hand-written ARIA is emitted here**, on the Combobox row's rule and for its reason: a static `aria-valuenow` that can never change replaces the user agent's correct semantics with a claim inert markup cannot keep. The radios and their captions are visually clipped rather than `display: none` — a hidden radio leaves the accessibility tree and the tab order, which would take away the very adjustability the floor exists to supply. **RECORDED KNOWN LIMIT — a writable rating whose current value is a FRACTION that lands on no enterable position (a bound 4.3 the reader may then overwrite) checks no radio here.** The floor shows the positions a reader can choose, not the average; the exact figure rides as `data-fuaran-rating-value` so it is visibly not dropped, and it is **not claimed as coverage** — nothing in the platform reads that attribute. Hydration restores the fraction. A filter chip renders the writable pair, `data-filter-name`-addressed. |
| **Input.Color** (Phase 1130) | A native `<input type="color">` carrying the resolved `#rrggbb` value — the whole control, on every tier, with no script and no ARIA of ours: the element carries its own role, its own keyboard model and the platform picker. **A value that resolves to something the element cannot hold falls back to the unset `#000000` rather than being passed through**, because a native colour input substitutes its own default silently, so handing it a bad literal would show a colour the document did not choose while the tree still said otherwise. This is the render path's share of a rule the decoder and the server-side submission floor also hold; none of the three coerces. A filter chip renders the same input, `data-filter-name`-addressed. |
| **Input.Tokens** (Phase 1121) | **One `<input type="text">` carrying the tokens comma-and-space separated**, inside the same `<span class="fuaran-tokens">` wrapper the client renders, optionally beside a `<datalist>` of the resolved suggestions. **There is no chip row here, and that is the conforming answer rather than a gap**: a chip row is BUILT by a keystroke handler — zero-JS there is no gesture that adds a chip and none that removes one — so a row of static chips with dead remove buttons would be an affordance inert markup cannot honour, which is the failure this whole family exists to avoid. What the floor renders is therefore a control a reader can actually EDIT and that submits with the form. The separator lives in `Fuaran.UI.Renderer.TokensModel` (`toCommaSeparated` / `fromCommaSeparated`), so the projection and its inverse cannot drift apart. **No hand-written ARIA is emitted here**, on the Combobox row's rule: the client tier's `role="combobox"` entry box is honest only because it owns the popup state, and a static claim of it would replace the user agent's own correct semantics with one inert markup can never keep. **TWO RECORDED KNOWN LIMITS, neither claimed as coverage.** (1) A token CONTAINING A COMMA does not survive the projection — it re-parses as two; escaping it would put a quoting grammar into a degraded medium no reader can see, trading a visible limit for an invisible one. (2) `allowFreeText = false` is NOT enforced here and cannot be — a text input has no native membership constraint — so the declaration rides as `data-fuaran-tokens-constrained` and nothing in the platform reads it; enforcement is the server-side re-check in `Fuaran.UI.ServerDriven.FormValidation`, which also re-checks uniqueness. A filter chip renders the same input, `data-filter-name`-addressed. |
...
| **Input.FileUpload** – `capture` (Phase 1116) | **The one declaration on this control whose floor FULLY holds — it is emitted, not degraded.** Unlike the two ingress routes on the row above, `capture` needs no listener and no script at all: it is an attribute the USER AGENT reads off the static markup, so a zero-JS document opens the platform camera or recorder exactly as a hydrated one does. This tier therefore emits the real HTML pair rather than a read-marker — `capture="environment"` for `Camera`, `capture="user"` for `Microphone` (both are conforming enumerated-attribute keywords; the device name itself is not one), beside `accept` from the declared filter. **`accept` rides with it because the keyword alone does not choose a device** — the capture request asks the platform for a recording device and `accept` decides which, so emitting one without the other would leave this floor holding only by accident. (`accept` was absent from this tier before Phase 1116, which nothing caught because nothing paired the two; the client tier has emitted it since Phase 130.) Neither attribute is synthesised from the other: an incoherent pair is REPORTED — FUARAN134, a Warning — and never repaired, because a renderer-invented filter is one the document did not write. A platform with no such device ignores `capture` and shows the file browser, so the degradation is the user agent's and the control stays a fully working upload. |
