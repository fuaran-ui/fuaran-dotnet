...
| **Display.Markdown** | **Real HTML via Markdig** (not the degraded `<pre>` the client's .NET fallback emits), routed through the `.Core` `Sanitize.sanitizeMarkdownHtml` seam. |
