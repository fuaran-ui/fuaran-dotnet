...
module GuestImport =
    ...
    let rec mapOpIds (f: NodeId -> NodeId) (op: TreeOp<'Msg>) : TreeOp<'Msg> =
        ...
        | TreeOp.InsertChild(parentId, position, child) -> TreeOp.InsertChild(f parentId, position, mapTreeIds f child)
        | TreeOp.RemoveNode id -> TreeOp.RemoveNode(f id)
        | TreeOp.MoveNode(id, newParentId, newPosition) -> TreeOp.MoveNode(f id, f newParentId, newPosition)
