...
// a browser host wraps `fetch`. Both implement the same minimal contract,
// so the mapper stays 100% pure and Fable-safe.
//
// These records carry only FSharp.Core + primitive types — no
// `System.Net.Http`, no `HttpRequestMessage` — so this file Fable-compiles
// alongside the rest of this package (the injected-interface posture;
...
/// address (the provider's `HttpClient.BaseAddress` on a .NET host), so a
/// mapper posts to e.g. "/v1/chat/completions" and never spells the host.
...
/// (a fully-read string body cannot carry an incremental stream, so a
/// streaming host reaches past this seam deliberately rather than by
/// widening it).
type IHttpTransport =
