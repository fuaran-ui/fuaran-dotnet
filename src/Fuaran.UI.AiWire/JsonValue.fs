...
///
/// Deliberately its own type rather than a reuse of `Fuaran.Core.Wire.JVal`:
/// this model has a null case (provider bodies use one) and a single numeric
/// type (as JSON does), where that one splits int from float and has no null.
/// The two are not interchangeable, and pretending otherwise would change
/// round-tripping at the provider boundary.
type JsonValue =
    | JNull
    | JBool of bool
    | JNumber of float
...
/// Ergonomic builders. Auto-opened so a `open Fuaran.UI.AiWire` lets a
