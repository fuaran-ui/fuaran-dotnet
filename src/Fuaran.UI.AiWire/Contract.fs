...
//
// This is a self-contained Apache-2.0 copy of the author's own portable
// AI-connector wire layer, published here under `Fuaran.UI.AiWire` so a
// Fuaran host can speak a provider's wire format without taking a
// dependency on any platform SDK. Apache-2.0 §4(b): modified copy,
// 2026-09-12.

// ─── Provider-level message types ────────────────────────────────
// These map to an AI API's own wire format. They are deliberately NOT a
// conversation-history model: a host that persists conversations owns
// that shape itself and projects into these types at the provider call.
...
/// which of those a given provider accepts is that provider's
/// documented business, not this contract's.
type ImageSource =
...
module AIContentPart =
    ...
    /// they are large (a typical phone photo is 1–5 MB base64) and
    /// frequently PII (faces, locations, sensitive documents).
    let redactedSummary (part: AIContentPart) : string =
        match part with
        | TextPart s ->
            // Keep the first 200 chars of text content. A long prompt can
            // still leak PII in that prefix; a host with a stricter
            // requirement redacts further upstream, before calling here.
...
/// Classification of AI provider failures. A provider adapter returns one
/// of these through its `Result` channel so a caller can reason about the
/// failure without string-matching a vendor message. One rule travels with
/// the vocabulary: a provider error is NEVER fed back to the model as a
/// tool result — it surfaces to the caller as a failed turn, because a
/// model handed a transport failure will invent a way to continue.
...
module AIProviderError =
    /// Human-readable rendering for logs, UI surfaces and error events.
    ...
    ///
    /// A CLASSIFICATION, and this package deliberately carries no loop to run
    /// it. Whether to retry is the HOST's call and cannot be made here: in a
    /// bring-your-own-key deployment an automatic retry spends the reader's own
    /// key, and the reader is the only party who can weigh that. Callers also
    /// use different rules at different levels — `StreamingAborted` is always
    /// terminal at the agent level, whatever this function says.
