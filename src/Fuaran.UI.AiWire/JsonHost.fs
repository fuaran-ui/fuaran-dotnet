...
/// and it matters here more than elsewhere, because the input is an
/// UNTRUSTED provider response.
///
/// **`serialize` is a single portable writer** shared by both hosts — it is
/// *not* `#if`-split, and that asymmetry with `parse` is the design rather
/// than an inconsistency. Two host JSON engines do not guarantee identical
/// number formatting, string escaping, or object key order, so a byte-parity
/// claim across hosts cannot rest on them agreeing. One hand-rolled writer,
/// the same F# compiled to both, makes it true by construction. Object
/// members emit in `JObject` list order; output is compact (no insignificant
/// whitespace).
...
module JsonHost =
    ...
    let rec private fromElement (el: JsonElement) : JsonValue =
        ...
        | JsonValueKind.String ->
            // `GetString()` is declared nullable, and returns null only for a null
            // element — which this match has already routed to `JNull`. Matched
            // rather than asserted, so the function stays total under F# 10
            // nullness without an assumption the compiler cannot check.
