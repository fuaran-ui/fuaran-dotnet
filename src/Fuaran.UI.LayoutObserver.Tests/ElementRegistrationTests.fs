module Fuaran.UI.LayoutObserver.Tests.ElementRegistrationTests
...
let tests =
    testList
        "layout-observer element registration"
        [ test "an id the registry does not hold is FRESH" {
          ...
          test "the browser observer routes registration through the classifier and clears the caches on a remount" {
              ...
              // browser behaves.
              let source =
                  Path.Combine(__SOURCE_DIRECTORY__, "..", "Fuaran.UI.LayoutObserver", "BrowserLayoutObserver.fs")
