...
let all: LenientFixture list =
    [
      ...
      { Id = "lenient-transform-step-aliases"
        ...
        VerboseJson =
          """{"id":"len-tf-a","kind":{"$type":"DataGrid","columns":[{"field":"dept","kind":{"$type":"Text"},"label":"Dept"}],"rowKeyField":"dept","source":{"$type":"Transform","pipeline":[{"$type":"groupBy","aggs":[{"fn":"mean","name":"avgPay","of":"salary"}],"keys":["dept"]},{"$type":"sort","by":[{"column":"avgPay","dir":"desc"}]},{"$type":"limit","n":3,"offset":0}],"source":{"columns":{"dept":{"validity":[true,true],"values":["ops","eng"]},"salary":{"validity":[true,true],"values":[100,200]}},"schema":[{"name":"dept","type":"string"},{"name":"salary","type":"int"}]}}}}"""
        Description =
          "fuaran-core#92 — the SQL/pandas step-field spellings (by/aggregations/op/as/avg, keys/descending, count) coerce to the canonical fields. The sort key's `column` is no longer one of them: `0.28.0` made that spelling CANONICAL and `col` the alias, so the verbose twin here moved with it" }
      ...
      { Id = "lenient-window-cumsum-legacy"
        ...
        VerboseJson =
          """{"id":"len-w-cs","kind":{"$type":"DataGrid","columns":[{"field":"running","kind":{"$type":"Text"},"label":"Running"}],"rowKeyField":"running","source":{"$type":"Transform","pipeline":[{"$type":"window","as":"running","fn":"cumulSum","of":"salary","orderBy":[{"column":"salary","dir":"asc"}],"partitionBy":["dept"]}],"source":{"columns":{"dept":{"validity":[true,true],"values":["ops","eng"]},"salary":{"validity":[true,true],"values":[100,200]}},"schema":[{"name":"dept","type":"string"},{"name":"salary","type":"int"}]}}}}"""
        Description = "2026-07-19 rename — the legacy cumSum window-fn tag coerces to the canonical cumulSum" }

      // ─── 0.28.0 — a column-naming member is spelled out in full ──────────
      // The rename runs the other way from every alias above it: `cols` and
      // `col` were CANONICAL and are now the aliases, so this pair is what
      // pins that a document written before the rename still decodes and
      // re-encodes to the new spelling. All three sites move together because
      // they are one rule and not three: a `project`'s rename list is
      // `columns`, and a sort key's and a window frame-ordering entry's column
      // is `column`. The `col` EXPRESSION's `$type` tag is deliberately absent
      // from that list — it names a KIND of expression rather than a column,
      // so it is untouched, and the `derive` step below carries one precisely
      // so a future reader can see the tag survive the member rename.
      { Id = "lenient-transform-column-member-legacy"
        LenientJson =
          """{"id":"len-tf-col","kind":{"$type":"DataGrid","columns":[{"field":"dept","kind":{"$type":"Text"},"label":"Dept"}],"rowKeyField":"dept","source":{"$type":"Transform","pipeline":[{"$type":"derive","expr":{"$type":"col","name":"salary"},"name":"pay"},{"$type":"sort","by":[{"col":"salary","dir":"desc"}]},{"$type":"window","as":"running","fn":"cumulSum","of":"salary","orderBy":[{"col":"salary","dir":"asc"}],"partitionBy":["dept"]},{"$type":"project","cols":[{"a":"dept","b":"dept"},{"a":"running","b":"total"}]}],"source":{"columns":{"dept":{"validity":[true,true],"values":["ops","eng"]},"salary":{"validity":[true,true],"values":[100,200]}},"schema":[{"name":"dept","type":"string"},{"name":"salary","type":"int"}]}}}}"""
        VerboseJson =
          """{"id":"len-tf-col","kind":{"$type":"DataGrid","columns":[{"field":"dept","kind":{"$type":"Text"},"label":"Dept"}],"rowKeyField":"dept","source":{"$type":"Transform","pipeline":[{"$type":"derive","expr":{"$type":"col","name":"salary"},"name":"pay"},{"$type":"sort","by":[{"column":"salary","dir":"desc"}]},{"$type":"window","as":"running","fn":"cumulSum","of":"salary","orderBy":[{"column":"salary","dir":"asc"}],"partitionBy":["dept"]},{"$type":"project","columns":[{"a":"dept","b":"dept"},{"a":"running","b":"total"}]}],"source":{"columns":{"dept":{"validity":[true,true],"values":["ops","eng"]},"salary":{"validity":[true,true],"values":[100,200]}},"schema":[{"name":"dept","type":"string"},{"name":"salary","type":"int"}]}}}}"""
        Description =
          "0.28.0 — the pre-rename column-member spellings decode unchanged and re-encode to the spelled-out names: a project step's `cols` becomes `columns`, and a sort key's and a window frame-ordering entry's `col` becomes `column`. The `col` expression tag is NOT a member name and does not move" }
