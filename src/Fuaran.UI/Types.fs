...
/// from the pre-swap hand-written definition): when `Label` is `None` NO `aria-label`
/// is emitted and there is no per-Kind structural-label fallback — the element's
/// accessible name then comes from its text content, which is where the renderer puts
/// the Node's structural label field (e.g. `ButtonSpec.Label`).
