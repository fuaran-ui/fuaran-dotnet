    ...
    testList
        ...
        [ test "Number formats with grouping + fixed fraction digits (en-US)" {
          ...
          test "Date reads whole Unix-epoch seconds" {
              let s =
                  Formatting.format "en-US" (Format.DateTime(Some DateStyle.Short, None)) 1700000000.0
          ...
          test "Format.DateTime with timeStyle alone renders a time of day and no date" {
              let s =
                  Formatting.format "en-GB" (Format.DateTime(None, Some TimeStyle.Short)) 1700000000.0
          ...
          test "Format.DateTime with both styles renders a date-time" {
              let s =
                  Formatting.format "en-GB" (Format.DateTime(Some DateStyle.Medium, Some TimeStyle.Short)) 1700000000.0

              Expect.stringContains s "2023" "the date portion"
              Expect.stringContains s "22:13" "the time portion"
          }

          test "Format.DateTime with dateStyle alone is byte-unchanged from the pre-1810 rendering" {
              let s =
                  Formatting.format "en-GB" (Format.DateTime(Some DateStyle.Short, None)) 1700000000.0
