...
let private boundMarkdown (id: string) (key: string) (dflt: string) : Node<obj> =
    ...
    { n with
        Kind = NodeKind.Display(DisplayKind.Markdown { Text = TextSource.Bound(Binding.State(key, dflt)) }) }
...
let private markdownLiteral (tree: Node<obj>) (id: string) : string option =
    ...
    | Some node ->
        ...
        | NodeKind.Display(DisplayKind.Markdown { Text = TextSource.Literal s }) -> Some s
        | _ -> None
    | None -> None
