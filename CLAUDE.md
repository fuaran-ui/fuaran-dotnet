...
reported as *not checked* when it is not — or when it is present but **older than its own renderer
sources**, since a build output nothing rebuilt describes a tree that has since moved, and the drift
it would report names a repair that regresses the embedded copy. See
[`docs/EMBEDDED-RENDERER.md`](docs/EMBEDDED-RENDERER.md).
