...
| `NodeKind.Sparkline` → `Charts.tryLowerSparkline` → `DrawingSvg.render` → `prop.dangerouslySetInnerHTML` (Phase 1098) | AI emission / wire decode — the resolved `float list` and **nothing else** | **No string seam at all.** The sparkline lowering emits one `Polyline` and no text: it writes no `Label`, no `title`/`desc`, no `Tip`, and its only author-derived values are the series NUMBERS, which reach the markup through `formatNum` (invariant; non-finite → `0`), never string interpolation. Its stroke and canvas are renderer-owned literals. So this seam inherits the `Drawing` row's escaping without relying on it — there is no untrusted string on the path. **A lowering that later gained a label or a tip would move to the `Drawing` row's posture and must say so here.** | `Fuaran.UI.Charts.tryLowerSparkline`; `Renderer.Core` `DrawingSvg` |
